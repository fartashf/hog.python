...
double* cv2c(const cv::Mat &img){
	...
	double *ret = new double[height*width*depth];
	for(int i = 0 ; i < height ; i++ ) {
		for(int j = 0 ; j < width ; j++ ) {
			for(int k = 0 ; k < depth ; k++ ) {
				ret[i + j*height + k*height*width] = double(data[k + j*depth + i*depth*width])/255;
			}
		}
	}
	return ret;
...
cv::Mat c2cv(const float *input, int *dims){
	...
	for(int i = 0 ; i < height ; i++ ) {
		for(int j = 0 ; j < width ; j++ ) {
			for(int k = 0 ; k < depth ; k++ ) {
				data[k + j*depth + i*depth*width] = (float)(input[i + j*height + k*height*width]);
			}
		}
	}
	return img;
...
float *computeHOG(double *im, int *dims, int *out, int sbin){
